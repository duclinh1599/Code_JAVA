...
public class NguoiDungController {
  ...
  public String index(final Model model, @RequestParam("page") Optional<Integer> pageNumber) {
    int _pageNumber = 1;
    if (pageNumber.isEmpty() == false) {
      _pageNumber = pageNumber.get();
    }
    System.out.println("pageNumber: " + pageNumber);
    QlvbDB qlvbDB = new QlvbDB();
    ArrayList<DmNguoiDung> arrNguoiDungs = qlvbDB.getNguoiDungPaging(_pageNumber, pageSize);

    //Tính tổng số trang:
    int iTongSoBanGhi = arrNguoiDungs.get(0).getTongSoBanGhi();
    int iTongSoPage =  (iTongSoBanGhi/pageSize) + 1 ;
    System.out.println("iTongSoPage: " + iTongSoPage);
    ArrayList<Integer> arrPages = new ArrayList<>();
    for(int i=1; i<= iTongSoPage; i++) {
      arrPages.add(i);
    }
