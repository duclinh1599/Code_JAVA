...
public class QlvbDB {
    ...
    public ArrayList<DmNguoiDung> getNguoiDungPaging(int pageNumber, int pageSize) {
        ArrayList<DmNguoiDung> arrResult = new ArrayList<>();
        try {
            int iLimit = pageSize;
            int iOffset = pageSize * (pageNumber - 1);
            String sqlText = "select  "
                    + "count(*) over() as TONG_SO_BAN_GHI, "
                    + "count(*) over(order by MA_NGUOI_DUNG) as SO_THU_TU,"
                    + "MA_NGUOI_DUNG, "
                    + "TEN_NGUOI_DUNG,"
                    + "MAT_KHAU,"
                    + "EMAIL,"
            ...
            while (rs.next()) {
                ...
                //Lấy tổng số bản ghi:
                dmNguoiDung.setTongSoBanGhi(rs.getInt("TONG_SO_BAN_GHI"));

                //Lấy số thứ tự:
                dmNguoiDung.setSoThuTu(rs.getInt("SO_THU_TU"));
            ...
            // TODO: handle exception
            System.out.println("getNguoiDungPaging co loi xay ra: " + e);
        }
        return arrResult;
    }
